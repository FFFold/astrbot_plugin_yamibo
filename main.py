...
class AstrBotPlugin(Star):
    ...
    async def help(self, event: AstrMessageEvent):
        ...
        text = (
            ...
            "\n"
            "【帖子与漫画】\n"
            "/yamibo 热帖 [N] — 今日热度榜（默认 10 条）\n"
            "/yamibo 版块 <版块|fid> [hot|new] [页] — 版块帖子列表\n"
            "/yamibo 搜索 <关键词> — 全站搜索帖子\n"
            "/yamibo 帖子 <tid|链接> — 帖子预览（摘要 + 前 3 张图）\n"
            "/yamibo 漫画 <tid|链接> [pdf|fwd|zip] — 打包发送漫画\n"
            "\n"
            "【订阅推送】\n"
            ...
            "\n"
            "【管理员】\n"
            "/yamibo 签到 [force] — 手动签到（force 跳过已签检查）\n"
            "/yamibo 签到状态 — 查看签到记录\n"
            "/yamibo 提醒 — 未读提醒数量\n"
            "/yamibo cookie状态 — 校验 cookie 有效性\n"
            "\n"
