...
def test_first_run_baseline_only_no_push():
...
def test_new_thread_same_day_pushed():
...
def test_already_pushed_same_day_not_repushed():
    state, _ = compute_incremental(None, _items(1, 2), today="2026-08-08")
    state, fresh = compute_incremental(state, _items(1, 2, 3), today="2026-08-08")
    assert [i.tid for i in fresh] == [3]
    # 掉榜又回来，同天内不重推
...
def test_ranking_shuffle_without_new_tid_no_push():
...
def test_next_day_resets_dedup():
    state, _ = compute_incremental(None, _items(1, 2), today="2026-08-08")
    state, fresh = compute_incremental(state, _items(1, 2, 3), today="2026-08-08")
    assert [i.tid for i in fresh] == [3]
    # 当天 3 掉榜
...
def test_cross_day_without_leave_no_repush():
    state, _ = compute_incremental(None, _items(1, 2), today="2026-08-08")
    state, fresh = compute_incremental(state, _items(1, 2, 3), today="2026-08-08")
    assert [i.tid for i in fresh] == [3]
...
def test_cross_day_keeps_last_tids_as_baseline():
    state, _ = compute_incremental(None, _items(1, 2), today="2026-08-08")
    state, fresh = compute_incremental(state, _items(2, 3), today="2026-08-09")
    # 1 掉榜、3 新进榜：只推 3
    assert [i.tid for i in fresh] == [3]
...
def test_empty_ranklist_keeps_baseline():
...
def test_empty_ranklist_first_run_keeps_no_baseline():
